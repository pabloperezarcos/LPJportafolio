...
export class RAtrasosPage implements OnInit {
  ...
  calcularMinutosAtraso(horaEntrada: string): number {
    // Establecer el horario de entrada a las 08:00
    const horarioEntrada = new Date(horaEntrada).setHours(8, 0, 0);

    // Obtener el timestamp de la hora de entrada
    const entrada = new Date(horaEntrada).getTime();

    // Calcular la diferencia en minutos
    const minutosAtraso = Math.max(0, Math.floor((entrada - horarioEntrada) / 60000));
    ...
    this.reportesService.getAsistenciasPorEmpleado(empleadoId).subscribe(
      (asistencias: any) => {
        ...
        this.asistenciasFiltradas = this.report.filter((asistencia) => {
          ...
          const tieneIngreso = horaEntrada && horaEntrada.trim() !== '';

          return (
            fechaRegistroTimestamp >= fechaInicioTimestamp &&
            fechaRegistroTimestamp <= fechaFinTimestamp &&
            tieneIngreso
          );
        });
